...
class Dataset(data.Dataset):
    def __init__(self, root, transform, target_transform, dataset_name, k_shots, save_dir, mode='train', seed=10, class_name=None):
        ...
        for cls_name, samples in meta_train_info.items():
            normal_samples = [item for item in samples if item.get('anomaly', 0) == 0]
            self.prompt_data_all[cls_name] = normal_samples if normal_samples else samples

        split_info = meta_train_info if self.mode == 'train' else meta_test_info
        if class_name is not None:
            self.cls_names = [class_name]
        else:
            self.cls_names = list(split_info.keys())
        ...
        for cls_name in self.cls_names:
          #  self.data_all.extend(split_info[cls_name])
            ...
            if len(self.view_list) > 1:
                for cls_name in self.cls_names:
                    ...
                    for view_id in self.view_list:
                        ...
                        data_view_tmp = [item for item in data_tmp if item['view_id'] == view_id and item.get('anomaly', 0) == 0]
                        if not data_view_tmp:
                ...
                for cls_name in self.cls_names:
                    data_tmp = meta_train_info[cls_name]
                    normal_data_tmp = [item for item in data_tmp if item.get('anomaly', 0) == 0]
                    data_tmp = normal_data_tmp if normal_data_tmp else data_tmp
                    torch.manual_seed(seed)
