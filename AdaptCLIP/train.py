...
def train(args):
    ...
    train_data = Dataset(root=args.train_data_path, transform=preprocess, target_transform=target_transform, \
                         dataset_name=dataset_name, k_shots=k_shots, save_dir=save_path, mode='train', seed=seed, class_name=args.class_name)
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for items in tqdm(train_data_loader):
            ...
            if args.visual_learner:
                ...
                global_loss += F.cross_entropy(global_logit, label.long())
            ...
            if args.pq_learner:
                ...
                for i in range(len(global_logit)):
                    global_loss += F.cross_entropy(global_logit[i], label.long())
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_data_path", type=str, default="../all-lpt", help="train dataset path")
    parser.add_argument("--save_path", type=str, default='./checkpoint/private_seed10', help='path to save results')
    parser.add_argument("--dataset", type=str, default='private_mvtec', help="train dataset name")
    ...
    parser.add_argument("--k_shots", type=int, default=0, help="how many normal samples")
    ...
    parser.add_argument("--class_name", type=str, help="optional single class name, for example foreign_object")
