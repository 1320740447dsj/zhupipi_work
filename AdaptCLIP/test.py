...
from tools.streaming_metric import StreamingEvaluator
from tools.streaming_excel import StreamingExcelWriter


def prompt_association(image_memory, patch_memory, target_class_name, device):
    ...
    for class_name in target_class_name:
        retrive_image.append(image_memory[class_name].to(device, non_blocking=True))  # S*D
        for l in range(patch_level_num):
            retrive_patch[l].append(patch_memory[class_name][l].to(device, non_blocking=True)) #
...
def build_prompt_memory(model, prompt_dataloader, device, obj_list, view_list, features_list, DPAM_layer,
    ...
    for idx, items in enumerate(tqdm(prompt_dataloader)):
        ...
        if store_on_cpu:
            ...
            patch_feat = [feature.cpu() for feature in patch_feat]
    ...
    if store_on_cpu:
        prompt_image_memory = {key: value.cpu() for key, value in prompt_image_memory.items()}
        prompt_patch_memory = {
            key: [value.cpu() for value in values]
            for key, values in prompt_patch_memory.items()
        }
...
def test(args):
    ...
    cpu_eva = args.cpu_evaluation
    if args.streaming_evaluation:
        if cpu_eva:
            raise ValueError('--streaming_evaluation and --cpu_evaluation cannot be used together')
        evaluator = StreamingEvaluator(device, metrics=eval_metrics, num_thresholds=args.streaming_thresholds)
        excel_results_path = Path(args.excel_results_path) if args.excel_results_path else Path(save_path) / 'streaming_results.xlsx'
        excel_writer = StreamingExcelWriter(
            excel_results_path,
            ['batch', 'sample_id', 'class_name', 'query_path', 'gt_anomaly', 'image_score', 'map_max', 'map_mean'],
            ['class_name'] + eval_metrics,
        )
    elif cpu_eva:
    ...
    if k_shots > 0:
        prompt_image_memory, prompt_patch_memory = build_prompt_memory(
            ...
            store_on_cpu=args.cpu_prompt_memory,
        )
    ...
    for idx, items in enumerate(tqdm(test_dataloader)):
        ...
        if k_shots > 0:
            ...
            prompt_feats, prompt_patch_feats = prompt_association(
                prompt_image_memory, prompt_patch_memory, target_cls_name, device
            )
        ...
        resize_mask = 256 if dataset_name in ['Real-IAD-Variety', 'RealIAD', 'bmad-medical'] else args.metric_image_size
        if resize_mask > 0 and pixel_anomaly_map.shape[-2:] != (resize_mask, resize_mask):
            pixel_anomaly_map = F.interpolate(
                pixel_anomaly_map[:, None], size=(resize_mask, resize_mask), mode='bilinear', align_corners=False
            )[:, 0]
            gt_mask = F.interpolate(gt_mask[:, None].float(), size=(resize_mask, resize_mask), mode='nearest')
            gt_mask = gt_mask[:, 0].bool().int()
        ...
        if args.streaming_evaluation:
            for current_class in dict.fromkeys(cls_name):
                class_indices = torch.tensor(
                    [name == current_class for name in cls_name], dtype=torch.bool, device=device
                )
                evaluator.update(
                    current_class,
                    pixel_anomaly_map[class_indices],
                    gt_mask[class_indices],
                    image_anomaly_pred[class_indices],
                    gt_anomaly[class_indices],
                )

            map_max = pixel_anomaly_map.flatten(1).amax(dim=1).detach().cpu().tolist()
            map_mean = pixel_anomaly_map.flatten(1).mean(dim=1).detach().cpu().tolist()
            image_scores = image_anomaly_pred.detach().float().cpu().tolist()
            image_targets = gt_anomaly.detach().int().cpu().tolist()
            for sample_index in range(current_batchsize):
                excel_writer.append_sample(
                    [
                        idx,
                        sample_id[sample_index],
                        cls_name[sample_index],
                        query_path[sample_index],
                        image_targets[sample_index],
                        image_scores[sample_index],
                        map_max[sample_index],
                        map_mean[sample_index],
                    ]
                )
        if args.streaming_evaluation:
            pass
    ...
    # ====================== Evaluation ======================
    if not args.streaming_evaluation:
    ...
    for idx, cls_name in enumerate(tqdm(obj_list)):
        metric_results = evaluator.compute(cls_name) if args.streaming_evaluation else evaluator.run(results_eval, cls_name, logger)
        if args.streaming_evaluation:
            excel_writer.append_metric([cls_name] + [metric_results[metric] * 100 for metric in eval_metrics])
    ...
    if args.streaming_evaluation:
        if len(obj_list) > 1:
            excel_writer.append_metric(['Avg'] + [msg[metric][-1] for metric in eval_metrics])
        excel_writer.close()
        logger.info(f'Excel results saved to: {excel_results_path}')
...
if __name__ == '__main__':
    ...
    # paths
    parser.add_argument("--test_data_path", type=str, default="./data", help="path to test dataset")
    ...
    parser.add_argument("--cpu_evaluation", action="store_true", help="store predictions and compute metrics on CPU")
    parser.add_argument("--cpu_prompt_memory", action="store_true", help="store few-shot prompt features on CPU")
    parser.add_argument("--metric_image_size", type=int, default=0, help="resize masks before pixel metrics; 0 keeps image_size")
    parser.add_argument("--streaming_evaluation", action="store_true", help="save each batch and accumulate metrics on GPU")
    parser.add_argument("--streaming_thresholds", type=int, default=512, help="number of thresholds for streaming pixel metrics")
    parser.add_argument("--excel_results_path", type=str, default="", help="output XLSX path for streaming results")
