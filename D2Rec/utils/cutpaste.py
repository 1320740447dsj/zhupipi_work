...
class PerlinPaste(object):
    def __init__(self, anomaly_dir=None):
        '''
        '''
        ...
        anomaly_dir = anomaly_dir or os.environ.get("D2REC_DTD_PATH")
        anomaly_dir = anomaly_dir or "/fuxi_team2/persons/danylgao/weicun_ceph/datasets/dtd/images"
        self.anomaly_paths = []
        for pattern in ("*/*.jpg", "*/*.jpeg", "*/*.png"):
            self.anomaly_paths.extend(glob.glob(os.path.join(anomaly_dir, pattern)))
        self.anomaly_paths = sorted(set(self.anomaly_paths))
        if not self.anomaly_paths:
            warnings.warn(
                "No DTD anomaly images found at {!r}; PerlinPaste will return the "
                "original image. Set D2REC_DTD_PATH or pass anomaly_path to enable "
                "Perlin augmentation.".format(anomaly_dir),
                RuntimeWarning,
            )
    ...
    def perlin_and_paste(self, normal_image, normal_mask=None):
        ...
        if not self.anomaly_paths:
            fallback_mask = np.zeros((w, h), dtype=np.uint8)
            return normal_image.copy(), fallback_mask
