...
import argparse
from utils.utils import cosine_loss_function, l2_normalize, cal_anomaly_map, get_gaussian_kernel, get_logger, \
    WarmCosineScheduler, visualize
...
def run_inference(data_loader, model, device, gaussian_kernel, image_size, crop_size, resize_mask, max_ratio,
                  mask_head, cache_on_device):
    ...
    for items in tqdm(data_loader):
        ...
        topk = int(resize_mask * resize_mask * max_ratio) if resize_mask is not None else int(
            crop_size * crop_size * max_ratio)
        if max_ratio <= 0:
    ...
    '''
    results_eval = dict(gt_masks=gt_masks, pr_masks=pr_masks, cls_names=cls_names, gt_anomalys=gt_anomalys,
                        pr_anomalys=pr_anomalys, img_paths=img_paths)
    results_eval = {k: np.concatenate(v, axis=0) if k == 'cls_names' or k == 'img_paths' else torch.cat(v, dim=0) for
                    k, v in results_eval.items()}
    return results_eval


def update_msg(msg, metric_results, cls_name, idx, obj_list, eval_metrics):
    ...
    print('{}: {}'.format(cls_name, ', '.join(
        ['{}: {:.1f}'.format(metric, cls_metric_msg[metric]) for metric in eval_metrics])))
...
def test(args):
    ...
    test_data = Dataset(root=args.data_path, transform=data_transform, target_transform=gt_transform,
                        image_size=image_size, dataset_name=args.dataset, mode='test')
    ...
    model = build_d2rec(encoder_name='dinov2reg_vit_base_14', dual_mask=args.dual_mask, dual_mask_type=dual_mask_type,
                        mask_head=args.mask_head, image_size=crop_size)

    model_path = save_path + '/dinov2_d2rec_30.pth'  # TODO: change to your model path
    ...
    if use_classwise_test:
        ...
        for idx, cls_name in enumerate(tqdm(obj_list)):
            ...
            cls_loader = torch.utils.data.DataLoader(cls_dataset, batch_size=args.batch_size, num_workers=8,
                                                     shuffle=False)
            results_eval = run_inference(
                cls_loader, model, device, gaussian_kernel, image_size, crop_size, resize_mask, max_ratio,
                args.mask_head, cache_on_device=True)
            metric_results = evaluator.run(results_eval, logger=logger)
            update_msg(msg, metric_results, cls_name, idx, obj_list, args.eval_metrics)

            del results_eval, cls_loader, cls_dataset
            torch.cuda.empty_cache()
    else:
        results_eval = run_inference(
            test_loader, model, device, gaussian_kernel, image_size, crop_size, resize_mask, max_ratio,
            args.mask_head, cache_on_device=len(test_data) < 5000)
        for idx, cls_name in enumerate(tqdm(obj_list)):
...
def train(args):
    ...
    save_path = args.save_path + args.dataset + '-' + str(args.image_size) + '-' + str(
        args.batch_size) + '-dualmask-' + str(args.dual_mask) + '-maskhead-' + str(args.mask_head)
    ...
    data_transform, gt_transform = get_data_transforms(image_size, crop_size)
    train_data = Dataset(root=args.data_path, transform=data_transform, target_transform=gt_transform,
                         image_size=image_size, dataset_name=args.dataset, mode='train', anomaly_path=args.anomaly_path)

    train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, num_workers=0, shuffle=True)
    ...
    print('fixed params:{:.1f}M, learned_params:{:.1f}M, all params:{:.1f}M'.format(fixed_num_params / 1e+6,
                                                                                    learned_num_params / 1e+6, (
                                                                                                fixed_num_params + learned_num_params) / 1e+6))
    logger.info('fixed params:{:.1f}M, learned_params:{:.1f}M, all params:{:.1f}M'.format(fixed_num_params / 1e+6,
                                                                                          learned_num_params / 1e+6, (
                                                                                                      fixed_num_params + learned_num_params) / 1e+6))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--anomaly_path", type=str, default=None,
                        help="DTD images root for Perlin augmentation (or set D2REC_DTD_PATH)")
    ...
    parser.add_argument("--eval_metrics", type=str, nargs="+", default=['I-AUROC', 'I-AP', 'P-AUROC', 'P-AP'],
