...
class PreProcessing:
    ...
    def __init__(self, device, resize_factor, brightness_augmentation = (1, 1), normalization='imagenet', target_size: int = 1024):
        self.device = device
    ...
    def __call__(self, x: torch.Tensor):
        ...
        # 若输入图像小于 target_size,先上采样到 target_size,保证 resize_factor 后尺寸 >= patch_size
        if x.shape[-2] < self.target_size or x.shape[-1] < self.target_size:
            x = torch.nn.functional.interpolate(x, size=(self.target_size, self.target_size), mode='bicubic', align_corners=False, antialias=True)
...
class DinoV3Backbone:
    ...
    def __init__(self, model_name: str, layers: list[int], device):
        # 优先使用本地缓存的 dinov3 源码,避免 torch.hub 调用 GitHub API 卡住
        local_repo_dir = os.path.join(torch.hub.get_dir(), 'facebookresearch_dinov3_main')
