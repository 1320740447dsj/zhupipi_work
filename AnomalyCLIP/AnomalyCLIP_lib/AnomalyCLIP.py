...
# [AnomalyCLIP改造点-1] 这里不是原始 CLIP 的 MultiheadAttention。
# AnomalyCLIP 在视觉 Transformer 的后几层做 DPAM/DAPM surgery：
# 保留原始 Q-K attention 路径用于全局图像特征，同时增加 V-V attention
# 异常定位路径，让 patch token 更偏向局部缺陷响应。
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 原始 CLIP 路径：仍使用 Q-K attention，后面用于 x_ori/global image feature。
        attn_ori = (q @ k.transpose(-2, -1)) * self.scale
        attn_ori = attn_ori.softmax(dim=-1)
        attn_ori = self.attn_drop(attn_ori)

        # AnomalyCLIP 异常定位路径：用 V 替换 K 和 Q，构造 V-V attention。
        k = v
        q = k

        # 该分支输出给 patch-level anomaly map，不再完全沿用 CLIP 的语义聚合方式。
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x, whole = False, ffn = False):
        # print("xxxxx",x.shape)
        # [AnomalyCLIP改造点-2] 被替换成 Attention 的深层视觉 block 会走双分支：
        # x     : V-V attention 异常定位分支，主要提供 patch token；
        # x_ori : 原始 Q-K attention 分支，保留 CLIP 全局语义特征。
        ...
        if isinstance(self.attn, Attention):
            if isinstance(x, list):
                if not ffn:
                    ...
                    x += x_res # 异常定位分支默认跳过 FFN，减少语义平滑对局部缺陷的影响。
                    # print('hellloooo')
                    return [x, x_ori]
                else:
                    x, x_ori_1 = x
...
class ResidualAttentionBlock_learnable_token(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # [AnomalyCLIP改造点-3] 文本 Transformer 支持深层可学习 prompt token。
        # prompt_learner 学到 normal/abnormal 的上下文 token；推理时这里把
        # 不同层的 compound prompt 注入文本序列，得到更适合异常检测的文本特征。
        # 对视觉中被 DPAM 替换的 block，仍沿用上面的双分支逻辑。
        if isinstance(self.attn, Attention):
            x = inputs[0]
            if isinstance(x, list):
                x, x_ori = x
...
class Transformer(nn.Module):
    ...
    def ori_CLIP_with_patch_forward(self, x, out_layers):
        # 原始 CLIP 前向的兼容路径：不做 DPAM，但额外收集中间层 patch token。
        idx = 0
        out_tokens = []
    ...
    def AnomalyCLIP_forward(self, x, out_layers, ffn):
        # AnomalyCLIP 视觉前向：深层 block 已被 DAPM_replace 替换成 V-V attention，
        # 这里收集指定层的异常定位分支 patch token，用于生成像素级热力图。
        idx = 0
        out_tokens = []
        for r in self.resblocks:
            idx += 1
    ...
    def forward(self, x: torch.Tensor, out_layers = [6, 12, 18, 24], DPAM_layer = None, ffn = False):
        ...
        if not self.text_layer:
            ...
            if DPAM_layer is None:
                ...
                # DPAM_layer 不为 None 时，视觉分支走 AnomalyCLIP 的 DPAM 改造路径。
...
class VisionTransformer(nn.Module):
    ...
    def DAPM_replace(self, DPAM_layer):
        # [AnomalyCLIP改造点-4] 直接对 CLIP ViT 的最后若干层做模块替换。
        # 原 MultiheadAttention 的 QKV/out_proj 权重被拷贝进自定义 Attention，
        # 因此可以继承预训练 CLIP 权重，同时改变后几层 patch token 的注意力形式。
        # DPAM_layer=20 表示从最后一层向前替换 19 个 block；DPAM_layer=1 等价于不替换。
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        # transformer 返回两类输出：
        # x_ori 是原始 attention 路径的 CLS/global feature；
        # patch_tokens 是指定层的 patch feature，后续和 normal/abnormal 文本特征做相似度。
...
class AnomalyCLIP(nn.Module):
    ...
    def encode_image(self, image, feature_list = [], ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        # AnomalyCLIP 的图像编码不只返回全局图像特征，还返回多层 patch 特征。
    ...
    def encode_text_learn(self, prompts, tokenized_prompts, deep_compound_prompts_text = None, normalize: bool = False):
        # 使用 prompt_learner 生成的可学习 normal/abnormal prompt，而不是固定文本模板。
