...
def generate_class_info(dataset_name):
    ...
    if dataset_name == 'mvtec':
        ...
        # 自定义单类别数据集（结构: data/{train/good, test/{good,bad}, ground_truth/bad}）
        obj_list = ['object']
...
class Dataset(data.Dataset):
    def __init__(self, root, transform, target_transform, dataset_name, mode='test'):
        ...
        # 跨平台兼容：Windows 路径分隔符为 '\'，不能用 split('/')
        ...
        if dataset_name == 'custom':
            # Custom datasets may contain any number of classes.
            self.obj_list = self.cls_names
            self.class_name_map_class_id = {
                cls_name: index for index, cls_name in enumerate(self.obj_list)
            }
        else:
