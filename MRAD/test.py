...
def test(args):
    ...
    pixel_aupro_list = []
    pixel_f1_list = []
    want_image = args.metrics in ('image-level', 'image-pixel-level')
    want_pixel = args.metrics in ('pixel-level', 'image-pixel-level')

    headers = ['objects']
    if want_pixel:
        headers.append('pixel_auroc')
        if args.compute_pixel_aupro:
            headers.append('pixel_aupro')
        headers.append('pixel_f1')
    if want_image:
        headers.extend(['image_auroc', 'image_f1', 'image_ap'])
    ...
    for obj in tqdm(obj_list):
        ...
        if want_image:
            ...
            table.extend([
                ...
                str(np.round(image_f1 * 100, decimals=1)),
    ...
    mean_row = ['mean']
    if want_pixel:
        mean_row.append(str(np.round(np.mean(pixel_auroc_list) * 100, decimals=1)))
        if args.compute_pixel_aupro:
            mean_row.append(str(np.round(np.mean(pixel_aupro_list) * 100, decimals=1)))
        mean_row.append(str(np.round(np.mean(pixel_f1_list) * 100, decimals=1)))
    if want_image:
        mean_row.extend([
    ...
    metric_table = tabulate(table_ls, headers=headers, tablefmt='pipe')
    logger.info("\n%s", metric_table)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cache_name", type=str, default=None,
                        help="source dataset name used in cache filenames")
    # Model parameters
