...
class MVTecADDataset(BaseDatasetHandler):
    ...
    def get_validation_paths(self):
        # Optional validation split (needed for F1 thresholds). Returns [] if absent.
        return sorted(glob.glob(str(self.category_path / "validation" / "good" / "*.png")))
...
class PrivateDataset(BaseDatasetHandler):
    """Handler for private MVTec-AD-style datasets with VisA-like mask naming.

    Structure (per category):
        category/
        ├── train/good/<img>              # normal training images
        ├── validation/good/<img>         # OPTIONAL normal images (needed for F1)
        ├── test/{good,bad}/<img>         # good = normal, bad = anomalous
        └── ground_truth/bad/<stem>.png   # mask for each bad test image (same stem)

    Supports multiple image extensions (png/jpg/jpeg/JPG/JPEG/bmp).
    Masks follow VisA naming: ground_truth/bad/{stem}.png (no "_mask" suffix).
    """

    _IMG_EXTS = ("*.png", "*.jpg", "*.jpeg", "*.JPG", "*.JPEG", "*.bmp")

    def _glob_imgs(self, *parts):
        results = []
        target = self.category_path.joinpath(*parts)
        for ext in self._IMG_EXTS:
            results.extend(glob.glob(str(target / ext)))
        return sorted(results)

    def get_train_paths(self):
        return self._glob_imgs("train", "good")

    def get_validation_paths(self):
        # Optional validation split (needed for F1 thresholds). Returns [] if absent.
        return self._glob_imgs("validation", "good")

    def get_test_paths(self):
        return self._glob_imgs("test", "*")

    def get_ground_truth_path(self, test_path: str):
        p = Path(test_path)
        if "good" in p.parts:
            return None
        # VisA-style mask naming: ground_truth/bad/{stem}.png
        mask_path = self.category_path / "ground_truth" / "bad" / f"{p.stem}.png"
        if mask_path.exists():
            return str(mask_path)
        # Fallback: any file sharing the same stem in ground_truth/bad/
        gt_dir = self.category_path / "ground_truth" / "bad"
        if gt_dir.exists():
            candidates = sorted(gt_dir.glob(f"{p.stem}.*"))
            if candidates:
...
def get_dataset_handler(name: str, root_path: str, category: str) -> BaseDatasetHandler:
    ...
    if name == "mvtec_ad":
        ...
        return PrivateDataset(root_path, category)
    else:
