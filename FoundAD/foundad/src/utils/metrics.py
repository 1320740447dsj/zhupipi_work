...
def calculate_pro(masks, scores, max_steps=200, expect_fpr=0.3, progress_desc=None):
    thresholds = np.linspace(scores.min(), scores.max(), max_steps)
    pros = []
    fprs = []

    threshold_iter = tqdm(
        thresholds, desc=progress_desc or "AUPRO", unit="threshold",
        leave=False, dynamic_ncols=True, disable=progress_desc is None,
    )
    ...
    if len(fprs) > 1:
        fpr_span = fprs.max() - fprs.min()
        if fpr_span > 0:
            fprs = (fprs - fprs.min()) / fpr_span
            return metrics.auc(fprs, pros)
        return float(np.max(pros))
    pro_auc = 0.0
