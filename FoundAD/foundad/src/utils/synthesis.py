...
_WARNED_UNKNOWN_SUBCLASSES = set()
...
def generate_target_foreground_mask(img: np.ndarray, subclass: str) -> np.ndarray:
    ...
    if subclass in ['carpet', 'leather', 'tile', 'wood', 'cable', 'transistor']:
        ...
        # Private MVTec-style categories have no known foreground heuristic.
        # Treat the full image as valid, as already done for texture classes.
        target_foreground_mask = np.ones_like(img_gray)
        if subclass not in _WARNED_UNKNOWN_SUBCLASSES:
            warnings.warn(
                f"Unknown category '{subclass}': using the full image for CutPaste placement.",
                RuntimeWarning,
                stacklevel=2,
            )
            _WARNED_UNKNOWN_SUBCLASSES.add(subclass)
