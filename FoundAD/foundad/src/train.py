...
class Trainer:
    def __init__(self, args: Dict[str, Any]):
        ...
        self.model = VisionModule(
            ...
            encoder_path=mcfg.get("encoder_path"), crop_size=mcfg["crop_size"],
        )
    ...
    def train(self):
        ...
        for ep in range(self.epochs):
            ...
            if self.scheduler is not None:
                self.scheduler.step()
        if gstep == 0:
            raise RuntimeError("No training batches were produced; check that train/ contains images")
        self._save_ckpt(self.epochs, gstep)
