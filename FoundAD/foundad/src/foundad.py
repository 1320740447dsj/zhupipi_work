...
class VisionModule(nn.Module):
    def __init__(self, model_name: str, pred_depth: int, pred_emb_dim: int, use_cuda: bool = True,
                 if_pe: bool = True, feat_normed: bool = False, encoder_path: str = None,
                 crop_size: int = 512):
        super().__init__()
        self.encoder_backend = "torch_hub"
        self.num_register_tokens = 0
        (self.encoder, self.num_patches, self.embed_dim, self.processor, self.projector) = self._build_encoder(
            model_name, encoder_path=encoder_path, crop_size=crop_size
        )
    ...
    def _build_encoder(self, model: str, encoder_path: str = None, crop_size: int = 512):
        ...
        if model == "dinov2":
            ...
            if encoder_path:
                ...
                enc = AutoModel.from_pretrained(
                    encoder_path, local_files_only=True, trust_remote_code=False
                ).eval()
                self.encoder_backend = "transformers"
                self.num_register_tokens = getattr(enc.config, "num_register_tokens", 0)
                patch_size = enc.config.patch_size
                num_patches = (crop_size // patch_size) ** 2
                embed_dim = enc.config.hidden_size
            else:
    ...
    def _extract(self, imgs: torch.Tensor, paths: List[str], n_layer: int = 3):
        if self.model_name == "dinov2":
            ...
            if self.encoder_backend == "transformers":
                output = self.encoder(pixel_values=imgs, output_hidden_states=True)
                h = output.hidden_states[-n_layer]
                h = h[:, 1 + self.num_register_tokens:, :]
            else:
