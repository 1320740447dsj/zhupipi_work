...
def process_main(rank: int, cfg_dict: dict, world_size: int):
    ...
    # A process group is unnecessary for the normal single-GPU path and can
    # fail on Windows builds compiled without libuv support.
    if world_size > 1:
...
def main(cfg: DictConfig):
    ...
    world_size = len(devices)
    if world_size == 1:
        process_main(0, cfg_dict, world_size)
        return
